...
function addToCart(productId) {
    ...
    console.log('Cart:', cart);  // Log the current cart
    console.log('Product ID:', productId);  // Log the product ID passed to the function
    console.log('Product Found:', product);  // Log the product that was found (or null if not found)
...
document.querySelectorAll('.add-to-cart').forEach(button => {
    button.addEventListener('click', function() {
        const productId = parseInt(button.dataset.productId);  // Ensure productId is a number
        console.log('Clicked Product ID:', productId);  // Log the ID when the button is clicked
        addToCart(productId);
    });
});
